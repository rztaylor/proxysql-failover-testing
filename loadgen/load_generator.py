...
QUERIES = {
    'small': [
        ...
        ("SELECT project_name, status, budget FROM projects WHERE project_id = %s", 
    ...
    'medium': [
        ...
        ("""SELECT p.project_name, p.budget, p.status, d.dept_name, 
            ...
            GROUP BY p.project_id, p.project_name, p.budget, p.status, d.dept_name""", lambda: ()),
    ],
    'large': [
        ...
        ("""SELECT e.emp_id, e.first_name, e.last_name, e.email, e.hire_date,
            ...
            ORDER BY d.dept_name, e.last_name""", lambda: ()),
        ...
        ("""SELECT p.project_name, p.status, p.budget,
            ...
            JOIN departments d ON e.dept_id = d.dept_id
            ORDER BY p.project_name, pa.role""", lambda: ()),
        ...
        ("""SELECT e.first_name, e.last_name, e.job_title, d.dept_name,
            ...
            JOIN departments d ON e.dept_id = d.dept_id
            ORDER BY sh.change_date DESC""", lambda: ()),
        ...
        ("""SELECT p.project_name, p.status,
            ...
            ORDER BY team_size DESC""", lambda: ()),
    ]
}
...
def execute_query(conn: mysql.connector.MySQLConnection, 
                  query: str, params: tuple) -> Tuple[bool, float, int]:
...
def main():
    ...
    config = load_config()
    logger.info(f"Configuration: base_rate={config['base_rate']} req/s, "
                f"jitter=±{config['rate_jitter']*100:.0f}%")
    ...
    try:
        while True:
            ...
            if success:
                ...
                logger.info(f"[{category.upper():6}] OK - {row_count:4} rows, "
                           f"{duration*1000:6.1f}ms - Query #{total_queries}")
            else:
                failed_queries += 1
                logger.warning(f"[{category.upper():6}] FAIL - {duration*1000:6.1f}ms - "
                              f"Query #{total_queries}")
            
            # Print stats every 50 queries
            if total_queries % 50 == 0:
                success_rate = (successful_queries / total_queries) * 100
                avg_duration = (total_duration / total_queries) * 1000
                logger.info(f"--- Stats: {total_queries} queries, "
                           f"{success_rate:.1f}% success, "
                           f"{avg_duration:.1f}ms avg ---")
        ...
        if total_queries > 0:
            success_rate = (successful_queries / total_queries) * 100
            avg_duration = (total_duration / total_queries) * 1000
            logger.info(f"Final stats: {total_queries} queries, "
